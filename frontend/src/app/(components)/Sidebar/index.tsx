  ...
  return (
    <div className={sidebarClassNames}>
        ...
        <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${isSidebarCollapsed ? "px-5" : "px-8"}`}>
            <div><Image src={logo} width={24} height={24} alt="main image" /></div>
            <h1 className={`${isSidebarCollapsed ? "hidden" : "block"} font-extrabold text-2xl`}>APSTOCK</h1>
        ...
        <div className={`${isSidebarCollapsed ? "hidden" : "block"} mb-10`}>
            <p className="text-center text-xs text-gray-500">&compy: 2024 APSTOCK</p>
        </div>
    </div>
  );
}
